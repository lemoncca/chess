    ...
    connect(ui->pushButton_5,&QPushButton::clicked,    //点击开始游戏，六子棋开始
            [this]()
            {
                ...
                if(pattern == 4 && is_listen)
                {
                    ...
                    {
                        order = 1;
                        QMessageBox::information(this,"先后手?","你抽到了先手！");
                    }
                    else
                    {
                        order = 2;
                        QMessageBox::information(this,"先后手?","你抽到了后手！");
                    }
                }
...
void MainWindow::reset_all()  //初始化
{
    ...
    if((pattern == 4 || pattern == 5) && is_listen && is_to_back_menu)
    {
        server->close();
        is_listen = 0;
        if(client_num > 0)
        ...
        QMessageBox::information(this,"监听提醒","监听已经结束！");
    }
    else if(pattern == 5 && is_listen && client_num == 2 && order == 0 && !is_to_back_menu )
    {
